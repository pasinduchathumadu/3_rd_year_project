...
function Pet_grooming() {
  ...
  const handleChange = (event) => {
    ...
    const selectedItem = timeslot.find(
      (item) => selectedValue === item.start + " - " + item.end
    );
    if (selectedItem) {
  ...
  const handleFormOpen = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/pet_care/user/date_client",
        {
          selectedDateString,
          Id,
        }
      ...
      try {
        ...
        if (res.data.message === "There is an internel error") {
          ...
          const res = await axios.get(
            `http://localhost:5000/pet_care/user/get_package/${package_id}`
          );
          const data = await res.data;
  ...
  const get_timeslot = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5000/pet_care/user/get_timeslot"
      );
      const data = await res.data;
      return data;
    } catch (err) {
  ...
  const get_package = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5000/pet_care/user/get_allpackages"
      );
      const data = await res.data;
      return data;
    } catch (err) {
                    ...
                    {timeslot &&
                      timeslot.map((name, index) => (
                        ...
                        >
                          <Checkbox
                            checked={
                              personName === name.start + " - " + name.end
                            }
                          />
                          <ListItemText
                            primary={name.start + " - " + name.end}
                          />
                        </MenuItem>
                      ))}
                  </Select>
            ...
            >
              ...
              >
                <CardActionArea>
                  ...
                  />
                  {package_care
                    .filter((menu, index) => menu.package_id === 1)
                    .map((menu) => (
                      <CardContent>
                        <Typography
                          gutterBottom
                        ...
                        >
                          <li className="tick-icon">
                            <CheckCircleIcon sx={{ color: "orange" }} /> Deep
                            Cleaning Shampoo
                          </li>
                          <li className="tick-icon">
                            <CheckCircleIcon sx={{ color: "orange" }} /> Blow
                            Dry
                          </li>
                          <li className="tick-icon">
                            <CheckCircleIcon sx={{ color: "orange" }} /> Ear
                            Cleaning
                          </li>
                          <li className="tick-icon">
                            <CheckCircleIcon sx={{ color: "orange" }} /> 15- Min
                            Brushout
                          </li>
                        </ol>
                        <Typography
                        ...
                        >
                          Clean grooming service without parabens,phthalates,
                          and chamical dyes
                        </Typography>
                        <div
                          style={{
                        ...
                        >
                          <Button
                            onClick={() => random_assit(menu.package_id)}
                            sx={{
                              backgroundColor: "black",
                              width: "90%",
              ...
              >
                ...
                >
                  <CardActionArea>
                    ...
                    <CardContent>
                      {package_care
                        .filter((menu, index) => menu.package_id === 2)
                        .map((menu) => (
                          <>
                            <Typography
                              gutterBottom
                      ...
                      >
                        ...
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Hair
                          Cuting And Styling
                        </li>
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Blow Dry
                        </li>
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Nail Trim
                        </li>
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Ear
                          Cleaning
                      ...
                      >
                        Clean grooming service without parabens,phthalates, and
                        chamical dyes
                      </Typography>
              ...
              >
                ...
                >
                  <CardActionArea>
                    ...
                    <CardContent>
                      {package_care
                        .filter((menu, index) => menu.package_id === 3)
                        .map((menu) => (
                          <>
                            <Typography
                              gutterBottom
                      ...
                      >
                        ...
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Sanitary
                          Trim
                        </li>
                        <li className="tick-icon">
                          <CheckCircleIcon sx={{ color: "orange" }} /> Body
                          Massage
                        </li>
                        <li className="tick-icon">
                    ...
                    {employee_detail &&
                      employee_detail.map((menu, index) => (
                        <img
                          key={index}
                          component="img"
                          src={getImageSrc(menu.img)} // Assuming getImageSrc is a function to get image source
                          alt="Down Arrow"
                          style={{
                            width: 200,
                            height: 200,
                ...
                <CardContent>
                  <Box>
                    ...
                    >
                      ...
                      >
                        {employee_detail &&
                          employee_detail.map((menu, index) => menu.full_name)}
                      </Typography>
                    </Stack>
