...
const StateNode = () => {
  ...
  const submit = async () => {
    const res = await axios.get(
      `http://localhost:5000/pet_care/user/generate/${id}`
    );
  ...
  const client_load = async () => {
    const res = await axios.get(
      `http://localhost:5000/pet_care/user/client_load/${id}`
    );
    const data = await res.data;
    return data;
  };
  ...
  const cancel = async (id) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/pet_care/user/delete_order/${id}`
      );
      ...
      {value === 0 && (
        <>
          <Grid
            container
            spacing={2}
            sx={{ marginTop: "3%", marginLeft: "38%", alignItems: "center" }}
          >
            <Grid item>
              <Paper
                elevation={3}
                style={circleStyle}
                sx={{
                  backgroundColor: state === "waitting" ? "green" : "white",
                  color: state === "waitting" ? "white" : "black",
                }}
              >
                {/* Content for State 1 */}
                Waitting
